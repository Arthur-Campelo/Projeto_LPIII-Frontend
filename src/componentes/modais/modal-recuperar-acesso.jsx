...
import { MostrarMensagemErro, checarListaVazia, validarCamposObrigatórios, validarConfirmaçãoSenha } from "../../utilitários/validações";
import {
    TAMANHOS, estilizarBotão, estilizarDivCampo, estilizarLabel, estilizarModal,
...
export default function ModalRecuperarAcesso() {
    ...
    const {
        cnpjVerificado, setCnpjVerificado, novaSenha,
        setNovaSenha, tokenRecuperação, setTokenRecuperação
    ...
    async function alterarSenha() {
        if (validarCampos()) {
            await serviçoAlterarUsuário({ cnpj: cnpjVerificado, senha: dados.senha, tokenRecuperação });

            setTokenRecuperação(null);
            mostrarToast(referênciaToast, "Senha redefinida com sucesso! Redirecionando ao Login...","sucesso");
        }
    };
